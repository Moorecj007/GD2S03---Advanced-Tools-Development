...
{
	// Fixing the Offset that the box separations created
	if (_iMouseX > 6 * m_iTileSize)
	{
		_iMouseX -= 4;
	}
	else if (_iMouseX > 3 * m_iTileSize)
	{
		_iMouseX -= 2;
	}
	if (_iMouseY > 6 * m_iTileSize)
	{
		_iMouseY -= 4;
	}
	else if (_iMouseY > 3 * m_iTileSize)
	{
		_iMouseY -= 2;
	}
...
void CGame::TextOutput()
{
	...
	else if (m_strSolveStatus == "UNSOLVABLE")
	{
		// RED text
	...
	TextOutA(m_pBackBuffer->GetBFDC(), 10, 680, m_strSolveStatus.c_str(), m_strSolveStatus.size());

	SetTextColor(m_pBackBuffer->GetBFDC(), RGB(255, 255, 255));

	std::string strSelected = "Selected -> ";

	std::string strInstructions = "Use number keys to select a number for placement";
	...
	std::string strPremade = "[F1] -> Generate the Premade Puzzle";
	std::string strGenerate = "[F2] -> Generate a new puzzle";
	...
	TextOutA(m_pBackBuffer->GetBFDC(), 10, 700, strSelected.c_str(), strSelected.size());

	TextOutA(m_pBackBuffer->GetBFDC(), 10, 730, strSolve.c_str(), strSolve.size());
	TextOutA(m_pBackBuffer->GetBFDC(), 10, 745, strPremade.c_str(), strPremade.size());
	TextOutA(m_pBackBuffer->GetBFDC(), 10, 760, strGenerate.c_str(), strGenerate.size());
	TextOutA(m_pBackBuffer->GetBFDC(), 10, 775, strReset.c_str(), strReset.size());

	TextOutA(m_pBackBuffer->GetBFDC(), 10, 805, strInstructions.c_str(), strInstructions.size());
	TextOutA(m_pBackBuffer->GetBFDC(), 10, 820, strLeftClick.c_str(), strLeftClick.size());
	TextOutA(m_pBackBuffer->GetBFDC(), 10, 835, strRightClick.c_str(), strRightClick.size());
}

/***********************
* PremadeProblem: Creates the premade problem and lays it on the grid
* @author: Callan Moore
* @return: void
********************/
void CGame::PremadeProblem()
{
	int* sudoku_2D[9];

	int row1[9] = { 8, 0, 0, 4, 0, 6, 0, 0, 7 };
	int row2[9] = { 0, 0, 0, 0, 0, 0, 4, 0, 0 };
	int row3[9] = { 0, 1, 0, 0, 0, 0, 6, 5, 0 };
	
	int row4[9] = { 5, 0, 9, 0, 3, 0, 7, 8, 0 };
	int row5[9] = { 0, 0, 0, 0, 7, 0, 0, 0, 0 };
	int row6[9] = { 0, 4, 8, 0, 2, 0, 1, 0, 3 };
	
	int row7[9] = { 0, 5, 2, 0, 0, 0, 0, 9, 0 };
	int row8[9] = { 0, 0, 1, 0, 0, 0, 0, 0, 0 };
	int row9[9] = { 3, 0, 0, 9, 0, 2, 0, 0, 5 };
...
/***********************
* InitialiseLua: Initialise the Enviroment/State for Lua
* @author: Callan Moore
* @return: void
********************/
...
/***********************
* ConvertTo2D: Converts a 1D sudoku puzzle into a 2D
* @author: Callan Moore
* @parameter: _sudoku1D: The current sudoku in 1D format
* @parameter: _sudoku2D: Storage variable for receiving the sudoku in 2D format
* @return: void
********************/
...
* ConvertTo2D: Converts a 1D sudoku puzzle into a 2D
* @author: Callan Moore
* @parameter: _sudoku2D: The current sudoku in 2D format
* @parameter: _sudoku1D: Storage variable for receiving the sudoku in 1D format
* @return: void
********************/
